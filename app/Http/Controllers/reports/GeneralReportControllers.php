...
class GeneralReportControllers extends Controller
{
    ...
    /*private function generateEntityActivityReport($query)
    {
        $activityReport = $query
            ...
            ->select(
                ...
                DB::raw('GROUP_CONCAT(DISTINCT CONCAT_WS("|", document.reference_code, document.received_date, document.sender_name, document.subject, document.file_path) SEPARATOR "||") as document_details')
            )
    ...
    {
        $departmentId = $request->input('department_id');
        return Office::where('department_id', $departmentId)
            ->select('id', 'name')
            ->get();
    }
   ...
   /* public function generatePdfReport(Request $request)
    {
        ...
        if (!$reportData->getData()->data) {
            return response()->json(['error' => 'No data available for PDF generation'], 400);
        }

        $pdf = PDF::loadView('reports.reports.pdftemplate', [
            'reportType' => $reportData->getData()->report_type,
            'headers' => $reportData->getData()->headers ?? [],
            'data' => $reportData->getData()->data
        ]);

        // Generar un nombre de archivo único
        $fileName = str_replace(' ', '_', strtolower($reportData->getData()->report_type)) . '_' . date('Y-m-d_H-i-s') . '.pdf';

        // Devolver el PDF directamente para su visualización en el navegador
